    ...
    public class DataBase
    {
        ...
        */
            ...
            using (var cmd = con.CreateCommand())
            {
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        {
                            var fitComSql = $"SELECT COM_ID,COM_NAME,COM_EXPIRATIONDATE ,COM_STATUS FROM COMMODITY WHERE STO_ID ={searchModel.sto_id} AND COM_STATUS !=-1 ORDER BY COM_RATING";
        ...
        public List<StoAddressListModel> sqlGetStorageAddress()
        {
            ...
            /*
            for (int i = 351; i < 382; i++)
            {
                try
                {
                    var folderpath = $".\\wwwroot\\commodity_image\\{i}";
                    var filepath = Path.Combine(folderpath, "com_image_0.jpg");
                    Directory.CreateDirectory(folderpath);
                    var imag = Image.FromFile("D:\\桌面\\ver2.0\\ver2.0\\backend\\DBproject\\wwwroot\\commodity_image\\261\\com_image_0.jpg");
                    imag.Save(filepath);
                    Console.WriteLine($"成功存入{filepath}");
                }
